jQuery(document).ready(function ($) {
    $("#SiteId").change(function () {
        var siteId = $(this).val();
        var applicationId = Number($('#ApplicationId').val());
        getSiteMapByParentId(applicationId, siteId);
    })

    $("#ApplicationId").change(function () {
        var applicationId = $(this).val();
        var siteId = Number($('#SiteId').val());
        getSiteMapByParentId(applicationId, siteId);
    })

    //var parentId = Number($('#ParentId').val());
    function getSiteMapByParentId(applicationId, siteId) {
        var data = {
            ApplicationId: applicationId,
            SiteId: siteId,
        };
        $.ajax({
            'url': '/api/Menu/GetByParentId',
            'method': 'POST',
            'dataType': 'json',
            'contentType': 'application/json',
            'data': JSON.stringify(data),
            'success': function (response) {
                if (response.code < 0) { //có lỗi
    ...
    const options = icons.map(cls => ({
        value: `<i class="ki-outline ki-${cls} fs-2"></i>`,
        text: cls
    }));

    // Thêm các tùy chọn vào select
    const $select = $("#selec2-icon");
    options.forEach(option => {
        $select.append(new Option(option.text, option.value));
    });
    $select.val(icon).trigger('change');
